...
from code_snippet_8 import MPLFigureEditor
...
class Camera(HasTraits):
    """ Camera objects. Implements both the camera parameter controls, and
    ...
    def acquire(self, experiment):
        X, Y = scipy.indices( (100, 100) )
        Z = scipy.exp( -((X-experiment.x)**2.0 + (Y-experiment.y)**2.) / experiment.width**2. )
        Z += 1-2.*scipy.rand(100,100)
        Z *= self.exposure
        Z[Z>2] = 2
        Z = Z**self.gain
        return(Z)
...
def process(image, results_obj):
    ...
    X, Y = scipy.indices(image.shape)
    x = sum(X*image)/sum(image)
    y = sum(Y*image)/sum(image)
    width = scipy.sqrt( abs( sum(((X-x)**2.+(Y-y)**2.)*image)/sum(image) ) )
    results_obj.x = x
    results_obj.y = y
